...
function calcularComissaoIndividual(valor: number): number {
    if (valor < 100) {
        return 0; 
    } else if (valor < 500) {
        return valor * 0.01; // 1%
...
const relatorioComissoes: Record<string, number> = {};

vendas.forEach((venda) => {
    const comissao = calcularComissaoIndividual(venda.valor);
    
    const totalAtual = relatorioComissoes[venda.vendedor] || 0;
    
    relatorioComissoes[venda.vendedor] = totalAtual + comissao;
});

console.log("=== RELATÓRIO DE COMISSÕES ===");
for (const vendedor in relatorioComissoes) {
    const total = relatorioComissoes[vendedor] || 0;
    console.log(`Vendedor: ${vendedor} | Total Comissão: R$ ${total.toFixed(2)}`);
}